...
class SS2D(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        #     nn.Conv2d(in_channels=d_model, out_channels=d_model, kernel_size=3, stride=1, padding=1),
        #     nn.SiLU(),
        #     nn.Conv2d(in_channels=d_model, out_channels=d_inner, kernel_size=3, stride=1, padding=1),
        # )
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        #ipdb.set_trace()
        #local=self.local_relation(x.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
        ...
        y = y * z
        #out = self.dropout(self.out_proj(y+local))
...
class SS2D_cross_new(nn.Module):
    def __init__(
        ...
        factory_kwargs = {"device": None, "dtype": None}
        super().__init__()
        d_expand = int(ssm_ratio * d_model1)
        d_inner = int(min(ssm_rank_ratio, ssm_ratio) * d_model1) if ssm_rank_ratio > 0 else d_expand
        self.dt_rank = math.ceil(d_model1 / 16) if dt_rank == "auto" else dt_rank
        self.d_state = math.ceil(d_model1 / 6) if d_state == "auto" else d_state  # 20240109
        self.d_conv = d_conv
        ...
        # in proj =======================================
        self.in_proj1 = nn.Linear(d_model1, d_expand * 2, bias=bias, **factory_kwargs)
        self.in_proj2 = nn.Linear(d_model2, d_expand * 2, bias=bias, **factory_kwargs)
        ...
        # out proj =======================================
        self.out_proj = nn.Linear(d_expand, d_model1, bias=bias, **factory_kwargs)
        self.dropout = nn.Dropout(dropout) if dropout > 0. else nn.Identity()
